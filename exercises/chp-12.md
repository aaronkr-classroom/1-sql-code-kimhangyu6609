...
2. 데이터웨어하우스, 데이터마트나 데이터마이닝에서는 수집된 데이터를 분석이 용이하도록 통일된 형태로 변환하는 과정을 ( 데이터 전처리 )이라고 한다.
...
SQL은 정형화된 데이터와 복잡한 트랜잭션 처리에 강점이 있고,

NoSQL은 유연한 스키마와 대용량 분산처리, 빠른 확장성에 강점이 있다.
