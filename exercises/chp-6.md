...
**17 인덱스의 장점과 단점을 설명하시오.**
 -읽기 위주 시스템에서는 인덱스 활용이 효과적
  쓰기 빈도가 높은 시스템에서는 신중히 사용해야 함
  필요한 컬럼에만 최소한으로 설정하는 것이 가장 좋습니다

**18 인덱스를 사용하는 것이 바람직한 경우와 그렇지 않은 경우를 구분하여 설명하시오.**
 -작업 유형	/ SELECT 중심 /	INSERT/UPDATE/DELETE 많음
  테이블 크기	/ 수천~수만 건 이상	/ 수십~수백 건 이하
  컬럼 특성	/ 검색*정렬에 자주 사용	/ 사용 빈도 낮거나 값 자주 변경
  목적	/ 성능 향상, 중복 방지	/ 공간 절약, 성능 유지
